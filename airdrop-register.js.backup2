...
function phantomHelpMessage() {
  if (isMobileDevice()) {
    return `Phantom is not available in this browser. Open this page from the Phantom app browser.<br><br>
    Phantom no está disponible en este navegador. Abre esta página desde el navegador interno de la app Phantom.`;
  }

  return `Phantom is not available. Install the Phantom extension and reload the page.<br><br>
  Phantom no está disponible. Instala la extensión de Phantom y recarga la página.`;
}
