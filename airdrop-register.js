...
export function mountAirdropRegister(selector = "#airdrop-register") {
    ...
    <div class="sf-airdrop-form">
      <div class="sf-steps-shell">
        <div class="sf-steps-head">
          <div>
            ...
            <div class="sf-steps-sub">Connect your wallet first, add your X + Telegram usernames second, then finish Cloudflare + Register.</div>
          </div>
        </div>
      ...
      <div class="sf-verify-shell">
        <div class="sf-verify-head">
          <div class="sf-verify-title">Step 2 · Social usernames</div>
          <div class="sf-verify-copy">Write your public <strong>X</strong> and <strong>Telegram</strong> usernames manually. Telegram is confirmed manually later during review.</div>
        </div>
        ...
        <div class="sf-field">
          ...
          <div id="sf-telegram-shell" class="sf-handle-shell">
            <span class="sf-prefix">@</span>
            <input id="sf-telegram" class="sf-input" placeholder="telegramusername" autocomplete="off" autocapitalize="off" spellcheck="false" />
          </div>
          <div class="sf-help">Write the same Telegram username that joined the community.</div>
        </div>

        <label style="display:flex;gap:10px;align-items:flex-start;padding:12px 14px;border-radius:16px;border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.03);color:#dbe7ff;line-height:1.5;cursor:pointer">
          <input id="sf-telegram-confirm" type="checkbox" style="margin-top:3px;accent-color:#4f8dff" />
          <span>I already joined the official Telegram community with this username.</span>
        </label>
      </div>
      ...
      <div id="sf-step3-shell" class="sf-verify-shell sf-hidden">
        <div class="sf-verify-head">
          ...
          <div class="sf-verify-copy">Complete <strong>Cloudflare</strong> and submit your entry.</div>
        </div>
  ...
  const stepInfo = {
    1: {
      ...
      copy: "Connect and sign your wallet first so the registration is tied to one verified Solana address.",
      points: [
    ...
    2: {
      ...
      copy: "Write both usernames manually using the accounts you want to use for the airdrop review.",
      points: [
        "Type your public <strong>X</strong> username.",
        "Type your public <strong>Telegram</strong> username.",
        "Confirm that you already joined the Telegram community."
      ]
    },
    3: {
      step: "Step 3",
      title: "Captcha + register",
      copy: "Complete Cloudflare and submit the registration for manual review.",
      points: [
        "Complete the <strong>Cloudflare</strong> check.",
        "Tap <strong>Register for Airdrop</strong>.",
        "Your entry stays <strong>pending review</strong> until approved."
      ]
    }
  };
  ...
  function getFieldState() {
    ...
    const telegramValid = TELEGRAM_HANDLE_RE.test(telegram);
  ...
  function clearMissingStep() { if (!lastMissingStep) return; lastMissingStep = 0; clearFieldHighlights(); updateStepCards(); }
  ...
  function refreshMessage() {
    ...
    const fields = getFieldState();
    if (registered) {
      setMsg("Airdrop registration sent and pending review.", "ok");
      return;
    }
    if (isSubmitting) {
